...
class NoReclamados extends Controller
{
    public function __construct()
    {
        session_start();
        parent::__construct();
        $this->model = new NoReclamadosModel();
        if (empty($_SESSION['activo'])) {
            header("location: ".base_url);
        }
        if (!verificar('No Reclamados')) {
    ...
    {
        $model = new NoReclamadosModel();
        $data['cajas'] = $model->getCajas();
        $data['roles'] = $model->getRoles();
        $this->views->getView('No Reclamados', "index", $data);
    }

    function listar()
    {
        $model = new NoReclamadosModel();
        $data = $model->getNoReclamados();
        if ($data) {
