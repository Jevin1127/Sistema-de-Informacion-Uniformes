...
class Usuarios extends Controller
{
    ...
    function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                ...
                if ($data[$i]['id'] == 1) {
                    $data[$i]["acciones"] = '<div>
                        <span class="badge bg-primary">Super Administrador</span>
                    </div>';
                } else {
    ...
    function validar()
    {
        ...
        if (empty($_POST['usuario']) || empty($_POST['clave'])) {
            ...
            $usuarioExiste = $model->verificarUsuario($usuario);
            if (!$usuarioExiste) {
                $msg = "El usuario que está tratando de ingresar no existe.";
            } else {
                ...
                if ($data) {
                    if ($data['estado'] == 1) {
                        if ($data['permisos'] != null){
                            ...
                            $acceso = $model->registrarAcceso($evento, $ip, $detalle);
                            if ($acceso > 0) {
                                $msg = "Datos Correctos";
                            } else {
                                $msg = "Error al capturar los datos del inicio";
                            }
    ...
    public function cambiarClave()
    {
        ...
        $nueva = isset($datos['nueva']) ? $datos['nueva'] : '';
        $confirmar = isset($datos['confirmar']) ? $datos['confirmar'] : '';
        ...
        if (empty($nueva) || empty($confirmar) || empty($token)) {
            ...
            if ($nueva != $confirmar) {
                ...
                if ($data === "El token no es válido.") {
                    $res = array('msg' => 'Token inválido', 'type' => 'warning');
                } elseif ($data === "La nueva contraseña no puede ser igual a la actual.") {
                    $res = array('msg' => 'La nueva contraseña no puede ser igual a la actual.', 'type' => 'warning');
                } elseif ($data === 1) {
