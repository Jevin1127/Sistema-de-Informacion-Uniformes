...
class EntregasModel extends Query {
    ...
    {
        $sql = "SELECT * FROM ventas WHERE estado = 0 ORDER BY saldo ASC";
...
public function getPedido($id) {
    ...
    $data = $this->selected($sql, [$id]);
    return $data;
}


public function abonarCantidad($id, $cantidadAbonada)
{
    $sql = "SELECT saldo FROM ventas WHERE id = ?";
    $data = $this->selected($sql, [$id]);
    if ($data) {
        $nuevoSaldo = $data['saldo'] - $cantidadAbonada;
        $sql = "UPDATE ventas SET saldo = ? WHERE id = ?";
        $result = $this->save($sql, [$nuevoSaldo, $id]);
...
public function NoEntregado(int $estado, int $id) 
{
    $this->estado = $estado;
    $this->id = $id;
    $sql = "UPDATE ventas SET estado = ? WHERE id = ?";
